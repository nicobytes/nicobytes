...
import chatExploImg from "./chat_explo_en.png";
import chatPuntualImg from "./chat_puntual_en.png";
...
<ArticleImage src={puntualImg} alt="Comparison of semantic, hybrid, and full text search for the precise query 'Grand Canyon South Rim': hybrid merges results from both layers" />
...
Where $d$ is the document, $\text{rank}_i(d)$ is the position it occupies in ranking $i$ (semantic or lexical), and $k$ is a smoothing constant (a typical value is $k = 60$). The intuition is straightforward: being in first position contributes $\frac{1}{k+1}$, second contributes $\frac{1}{k+2}$, and so on; the lower it appears, the less it adds.
...
<ArticleImage src={chatExploImg} alt="Amaru agent chat: the user asks for warm-weather getaways and the agent calls search_context to recommend Hawaii, San Diego, and Key West" />
...
<ArticleImage src={chatPuntualImg} alt="Amaru agent chat: the user asks about Grand Canyon South Rim tours and search_context returns South Rim, North Rim, and Rim-to-Rim routes" />
